...
|---|---|
| **`:app`** | Compose UI, ViewModels, MVI State/Intent/Effect, **Navigation 3**, Theme |
...
The `qa` build type is specifically configured with `isDebuggable = false` and inherits release ProGuard/R8 rules to ensure QA testing happens on code that is as close to production as possible.
