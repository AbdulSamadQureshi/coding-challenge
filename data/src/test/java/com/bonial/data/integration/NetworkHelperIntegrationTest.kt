...
class NetworkHelperIntegrationTest {
    ...
    @Test
    fun `safeApiCall emits loading then success for a 200 response`() =
        runBlocking {
    ...
    @Test
    fun `safeApiCall emits loading then error with code 404 for a not-found response`() =
        runBlocking {
    ...
    @Test
    fun `safeApiCall emits loading then error with server message for a 500 response`() =
        runBlocking {
    ...
    @Test
    fun `withRetry retries on 429 and succeeds when the server eventually returns 200`() =
        runBlocking {
    ...
    @Test
    fun `withRetry throws after exhausting max attempts on persistent 429 responses`() =
        runBlocking {
    ...
    @Test
    fun `withRetry does not retry on non-retryable errors and throws immediately`() =
        runBlocking {
            mockWebServer.enqueue(MockResponse().setResponseCode(500))
