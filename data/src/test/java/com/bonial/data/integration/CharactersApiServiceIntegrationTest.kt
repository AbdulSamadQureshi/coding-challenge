...
class CharactersApiServiceIntegrationTest {
    ...
    @Test
    fun `fetching a character page deserialises page info and character list correctly`() =
        runBlocking {
    ...
    @Test
    fun `fetching a character page sends the correct page and name query parameters`() =
        runBlocking {
            mockWebServer.enqueue(MockResponse().setBody(CHARACTERS_PAGE_JSON).setResponseCode(200))
    ...
    @Test
    fun `fetching a character page without a name omits the name query parameter`() =
        runBlocking {
            mockWebServer.enqueue(MockResponse().setBody(CHARACTERS_PAGE_JSON).setResponseCode(200))
    ...
    @Test
    fun `fetching a single character by id deserialises all fields correctly`() =
        runBlocking {
    ...
    @Test
    fun `fetching a single character by id sends the id in the path`() =
        runBlocking {
            mockWebServer.enqueue(MockResponse().setBody(CHARACTER_DETAIL_JSON).setResponseCode(200))
    ...
    @Test
    fun `character with all optional fields absent deserialises without crashing`() =
        runBlocking {
    ...
    @Test
    fun `character page with null results array deserialises to a null results field`() =
        runBlocking {
    ...
    @Test
    fun `http 404 response throws an HttpException with code 404`() =
        runBlocking {
            mockWebServer.enqueue(MockResponse().setResponseCode(404))

            val exception =
                runCatching { apiService.characters(page = 1) }
                    .exceptionOrNull()
    ...
    @Test
    fun `http 500 response throws an HttpException with code 500`() =
        runBlocking {
            mockWebServer.enqueue(MockResponse().setResponseCode(500))

            val exception =
                runCatching { apiService.character(id = 1) }
                    .exceptionOrNull()
